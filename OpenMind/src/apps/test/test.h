...
namespace grape
{
  ...
  public:
                                             ...
                                             const std::string &outfile_str)
    {
      ...
      static_assert(std::is_same_v<policy, GUNDAM::execution::sequenced_policy> || std::is_same_v<policy, GUNDAM::execution::parallel_policy>, "illegal policy");
      ...
      const size_t kEachParitionNum = (multi_topo_seq.size() + each_parition_size - 1) / each_parition_size;
                                              ...
                                              const std::string &outfile_str)
    {
      ...
      for (int i = 0; i <= loop_num; i++)
      {
        ...
        std::ofstream outlog(outfile_str);
        for (const auto &[query_vertex, candidate_counter] : candidate_counter_set)
        {
    ...
    void WriteTriple(const std::string kGarSetName)
    {
      ...
      for (const auto &gar : gar_list)
      {
        ...
        for (auto edge_it = gar_with_triple.EdgeBegin();
             ...
             edge_it++)
        {
          const std::string kTriple = GUNDAM::ToString(edge_it->src_handle()->label()) + " " + GUNDAM::ToString(edge_it->label()) + " " + GUNDAM::ToString(edge_it->dst_handle()->label());
                          ...
                          bool make_id_continued = false)
    {
      ...
      for (size_t row = 0; row < movie_size; row++)
      {
        ...
        max_movie_id = max_movie_id > movie_id[row] ? max_movie_id : movie_id[row];
        auto [vertex_handle,
      ...
      for (size_t row = 0; row < rating_size; row++)
      {
        ...
        auto [edge_handle,
              edge_ret] = graph.AddEdge(rating_user_id[row] + user_id_offset,
                                        rating_movie_id[row],
                                        // rating_rating  [row] * 2,
                                        rating_rating[row] >= kRatingThreadhold ? kHighReviewLabel
                                                                                : kLowReviewLabel,
                                        edge_id_counter++);
        if (!edge_ret)
        {
                     ...
                     const std::string &city)
    {
      ...
      std::vector<std::string> business_id = business_csv_file.GetColumn<std::string>(kBusinessBusinessIdColName);
      std::vector<std::string> business_categories = business_csv_file.GetColumn<std::string>(kBusinessCategoriesColName);
      std::vector<std::string> business_city = business_csv_file.GetColumn<std::string>(kBusinessCityColName);
      std::vector<std::string> business_name = business_csv_file.GetColumn<std::string>(kBusinessNameColName);
      std::vector<float> business_stars = business_csv_file.GetColumn<float>(kBusinessStarsColName);
      ...
      for (size_t row = 0; row < business_id.size(); row++)
      {
        ...
        auto [vertex_handle,
              vertex_ret] = graph.AddVertex(kVertexId, kBusinessLabel);
        assert(vertex_ret);
      ...
      std::vector<std::string> review_user_id = review_csv_file.GetColumn<std::string>(kReviewUserIdColName);
      std::vector<std::string> review_business_id = review_csv_file.GetColumn<std::string>(kReviewBusinessIdColName);
      std::vector<std::string> review_id = review_csv_file.GetColumn<std::string>(kReviewReviewIdColName);
      std::vector<double> review_stars = review_csv_file.GetColumn<double>(kReviewStarsColName);
      ...
      for (size_t row = 0; row < review_user_id.size(); row++)
      {
        ...
        auto [review_user_id_it,
              review_user_id_ret] = user_id_map.emplace(kReviewUserId, business_id_map.size() + user_id_map.size());
      ...
      std::vector<std::string> tips_user_id = tips_csv_file.GetColumn<std::string>(kTipsBusinessIdColName);
      std::vector<std::string> tips_business_id = tips_csv_file.GetColumn<std::string>(kTipsUserIdColName);
      ...
      for (size_t row = 0; row < tips_user_id.size(); row++)
      {
        ...
        auto [tips_user_id_it,
              tips_user_id_ret] = user_id_map.emplace(kTipsUserId, business_id_map.size() + user_id_map.size());
        if (tips_user_id_ret)
        {
          // this user has not been considered before
          auto [vertex_handle,
      ...
      std::vector<std::string> user_id = user_csv_file.GetColumn<std::string>(kUserUserIdColName);
      std::vector<std::string> user_friends = user_csv_file.GetColumn<std::string>(kUserFriendsColName);
                ...
                const std::string &rule_posfix) const
    {
      ...
      {
        const bool isPrefix = rule_prefix.size() <= gar_file.size() && std::mismatch(rule_prefix.begin(),
                                                                                     rule_prefix.end(),
                                                                                     gar_file.begin(),
                                                                                     gar_file.end())
                                                                               .first == rule_prefix.end();
        const bool isGarPosfix = rule_posfix.size() <= gar_file.size() && std::mismatch(rule_posfix.begin(),
                                                                                        rule_posfix.end(),
                                                                                        gar_file.end() - rule_posfix.size(),
                                                                                        gar_file.end())
                                                                                  .first == rule_posfix.end();
        ...
        gar_file = gar_file.substr(0, gar_file.length() - rule_posfix.size());
                        ...
                        std::string table_attr_name)     // "volume"
    {
      ...
      std::vector<std::string> output_article_col_name = output_article_csv.GetColumnNames();
      ...
      const std::vector<std::string> q_DM_DG_output_csv_column_name = q_DM_DG_output_csv.GetColumnNames();
      ...
      auto temp_q_DM_DG_output_csv_colum_names = temp_q_DM_DG_output_csv.GetColumnNames();
                      ...
                      std::string output_e_file)
    {
      ...
      for (size_t idx = 0; idx < kColumnNameSet.size(); idx++)
      {
        ...
        auto [edge_name_vertex_label_map_it,
              edge_name_vertex_label_map_ret] = edge_name_vertex_label_map.emplace(std::get<0>(kColumnNameSet[idx]),
      ...
      std::vector<VertexIdType> vertex_id_set = profiles_file_csv.GetColumn<VertexIdType>(vertex_id_column_idx);
      ...
      for (const auto &vertex_id : vertex_id_set)
      {
        ...
        vertex_id_counter = vertex_id_counter > vertex_id ? vertex_id_counter : vertex_id;
           ...
           column_idx++)
      {
        ...
        const std::vector<std::string> attr_set = profiles_file_csv.GetColumn<std::string>(column_idx);
        ...
        for (size_t row_idx = 0; row_idx < attr_set.size(); row_idx++)
        {
          ...
          assert(kAsVertex);
          auto attr_vertex_id_map_it = attr_vertex_id_map.find(kAttrValue);
          if (attr_vertex_id_map_it == attr_vertex_id_map.end())
          {
            // does not contain this value before
            attr_vertex_id_map_it = attr_vertex_id_map.emplace_hint(attr_vertex_id_map_it,
                                                                    kAttrValue,
                                                                    vertex_id_counter++);
            assert(column_name_vertex_label_map.find(kColumnName) != column_name_vertex_label_map.end());
            auto [vertex_handle,
          ...
          assert(attr_vertex_id_map_it != attr_vertex_id_map.end());
      ...
      const std::vector<VertexIdType> src_id_set = relationships_file_csv.GetColumn<VertexIdType>(src_id_column_idx);

      const std::vector<VertexIdType> dst_id_set = relationships_file_csv.GetColumn<VertexIdType>(dst_id_column_idx);

      assert(src_id_set.size() == dst_id_set.size());

      for (size_t row = 0; row < src_id_set.size(); row++)
      {
        assert(column_name_vertex_label_map.find(kRelationEdge) != column_name_vertex_label_map.end());
