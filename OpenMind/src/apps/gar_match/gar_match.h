#ifndef EXAMPLES_ANALYTICAL_APPS_GAR_MATCH_GAR_MATCH_H_
#define EXAMPLES_ANALYTICAL_APPS_GAR_MATCH_GAR_MATCH_H_
...
#include "gar_match/gar_match_context.h"
...
class GarMatch : public ParallelAppBase<FRAG_T, GarMatchContext<FRAG_T>>,
 ...
 private:
  ...
  inline std::pair<std::tuple<std::string,
                              ...
                              std::string,
                              std::string>,
                   bool> GetGarPathInfo(const YAML::Node& gar_path_config) const {
    ...
    if (gar_path_config["PivotId"]) {
      gar_pivot_id_str = gar_path_config["PivotId"].as<std::string>();
    }
  ...
  static constexpr std::string_view kWorkerGarMatchPosfix = "@worker";

  static constexpr std::string_view kRulePosfix    = GarMatchContext<FRAG_T>::kRulePosfix;
  static constexpr std::string_view kRuleSetPosfix = GarMatchContext<FRAG_T>::kRuleSetPosfix;

  static constexpr std::string_view kSuppPosfix    = GarMatchContext<FRAG_T>::kSuppPosfix;
  static constexpr std::string_view kSuppSetPosfix = GarMatchContext<FRAG_T>::kSuppSetPosfix;
                  
  using    VertexIDType = typename GarMatchContext<FRAG_T>::   VertexIDType;
  using VertexLabelType = typename GarMatchContext<FRAG_T>::VertexLabelType;
  using      EdgeIDType = typename GarMatchContext<FRAG_T>::     EdgeIDType;
  using   EdgeLabelType = typename GarMatchContext<FRAG_T>::  EdgeLabelType;
  using    DataGraph    = typename GarMatchContext<FRAG_T>::DataGraph;
  using      Pattern    = typename GarMatchContext<FRAG_T>::  Pattern;
  using      GarType    = typename GarMatchContext<FRAG_T>::  GarType;
  ...
  using CandidateSet = typename GarMatchContext<FRAG_T>::CandidateSet;
 ...
 public:
  ...
  INSTALL_PARALLEL_WORKER(GarMatch<FRAG_T>, 
                          GarMatchContext<FRAG_T>,
                          FRAG_T)
  using vertex_t = typename fragment_t::vertex_t;

  /**
   * @brief Partial evaluation for GarMatch.
   *
   * @param frag
   * @param ctx
   * @param messages
  ...
  void PEval(const fragment_t& frag, 
    ...
    if (!config["GarPath"]) {
      util::Error("cannot get gar path");
      return;
    }
    YAML::Node gar_path_config = config["GarPath"];

    if (gar_path_config.IsSequence()) {
      ctx.gar_path_set_.reserve(gar_path_config.size());
      for (int i = 0; i < gar_path_config.size(); i++) {
        auto [ gar_path_string, 
               gar_path_ret ] = GetGarPathInfo(gar_path_config[i]);
        if (!gar_path_ret) {
          util::Error("load gar path error!");
          return;
        }
        if (gar_path_config[i]["RhsMatchFile"]) {
          if (ctx.frag_num_ > 1) {
            util::Error("only support export rhs match result with single worker");
            return;
          }
          const std::string kRhsMatchFile = gar_path_config[i]["RhsMatchFile"].as<std::string>();
          ctx.gar_path_set_.emplace_back(gar_path_string, kRhsMatchFile);
          continue;
        }
        ctx.gar_path_set_.emplace_back(gar_path_string, "");
      }
    }
    else {
      auto [ gar_path_string, 
             gar_path_ret ] = GetGarPathInfo(gar_path_config);
      if (!gar_path_ret) {
        util::Error("load gar path error!");
        return;
      }
      if (gar_path_config["RhsMatchFile"]) {
        if (ctx.frag_num_ > 1) {
          util::Error("only support export rhs match result with single worker");
          return;
        }
        const std::string kRhsMatchFile = gar_path_config["RhsMatchFile"].as<std::string>();
        ctx.gar_path_set_.emplace_back(gar_path_string, kRhsMatchFile);
      }
      else {
        ctx.gar_path_set_.emplace_back(gar_path_string, "");
    ...
    if (ctx.gar_path_set_.size() == 0) {
      util::Error("empty gar set");
      return;
    }
    
    ctx.gar_set_.reserve(ctx.gar_path_set_.size());
    assert(ctx.gar_set_.empty());

    for (const auto& [kGarName, kRhsMatchFile] : ctx.gar_path_set_) {

        const auto& [kGarPathVFile, 
                     kGarPathEFile, 
                     kGarPathXFile, 
                     kGarPathYFile, 
                          kPivotId] = kGarName;
      ...
      ctx.gar_set_.emplace_back(std::move(gar), PatternVertexHandle());

      assert(!ctx.gar_set_.back().second);

      auto& gar_ref = ctx.gar_set_.back().first;

      #ifndef NDEBUG
      for (const auto& x_literal_ptr : gar_ref.x_literal_set()) {
        std::vector<PatternVertexHandle> pivot_set;
        x_literal_ptr->CalPivot(pivot_set); 
        for (const auto& pivot_handle : pivot_set) {
          assert(gar_ref.pattern().FindVertex(pivot_handle->id()) == pivot_handle);
        }
      }

      for (const auto& y_literal_ptr : gar_ref.y_literal_set()) {
        std::vector<PatternVertexHandle> pivot_set;
        y_literal_ptr->CalPivot(pivot_set); 
        for (const auto& pivot_handle : pivot_set) {
          assert(gar_ref.pattern().FindVertex(pivot_handle->id()) == pivot_handle);
        }
      }
      ...
      if (kPivotId != "") {
        const VertexIDType kGarPivotId = GUNDAM::StringToDataType<VertexIDType>(kPivotId);

        Pattern rhs_pattern(gar::_gar_supp::LiteralPattern(gar_ref, true));

        if (!rhs_pattern.FindVertex(kGarPivotId)) {
          ...
          util::Error("cannot find the pivot vertex with id: " + GUNDAM::ToString(kGarPivotId)
                      + " in rhs pattern contains of vertex: " + rhs_pattern_vertex_set);
          return;
        }
        // gar_ref.pattern().FindVertex(kGarPivotId)
        //     != rhs_pattern.FindVertex(kGarPivotId)
        // need to find it from graph pattern again
        ctx.gar_set_.back().second = gar_ref.pattern().FindVertex(kGarPivotId);
      }
      else {
        // randomly select one from rhs_pattern
        Pattern rhs_pattern(gar::_gar_supp::LiteralPattern(gar_ref, true));

        std::vector<VertexIDType> rhs_gar_vertex_id_set;
        for (auto vertex_it = rhs_pattern.VertexBegin();
                 !vertex_it.IsDone();
                  vertex_it++) {
          rhs_gar_vertex_id_set.emplace_back(vertex_it->id());
        }
        std::sort(rhs_gar_vertex_id_set.begin(),
                  rhs_gar_vertex_id_set.end());
        assert(!rhs_gar_vertex_id_set.empty());

        ctx.gar_set_.back().second = gar_ref.pattern().FindVertex(*rhs_gar_vertex_id_set.begin());
      }
      assert(ctx.gar_set_.back().second);
    }

    #ifndef NDEBUG
    for (const auto& [gar, pivot] : ctx.gar_set_) {
      assert(pivot);
      assert(pivot == gar.pattern().FindVertex(pivot->id()));
    }
    #endif // NDEBUG
    ...
    ctx.x_supp_limit_ = -1;
    if (config["XSuppLimit"]) {
      ctx.x_supp_limit_ = config["XSuppLimit"].as<int64_t>();
      util::Info("XSuppLimit: " + std::to_string(ctx.x_supp_limit_));
    }

    ctx.xy_supp_limit_ = -1;
    if (config["XYSuppLimit"]) {
      ctx.xy_supp_limit_ = config["XYSuppLimit"].as<int64_t>();
      util::Info("XYSuppLimit: " + std::to_string(ctx.xy_supp_limit_));
    }
  ...
  /**
   * @brief Incremental evaluation for GarMatch.
   *
   * @param frag
   * @param ctx
  ...
  void IncEval(const fragment_t& frag, 
    ...
    std::function<bool(const GUNDAM::Match<Pattern, DataGraph>&)> 
      satisfy_x_supp_collect_output_callback 
         = [&ctx](const GUNDAM::Match<Pattern, DataGraph>& match) -> bool {
      ctx.match_set_.AddMatch(match);
      if (ctx.x_supp_limit_ == -1){
        // x_supp_limit is not specified 
        // continue matching
        return true;
      }
      assert(ctx.match_set_.size() < ctx.x_supp_limit_);
      if (ctx.match_set_.size() == ctx.x_supp_limit_){
    ...
    std::function<bool(const GUNDAM::Match<Pattern, DataGraph>&)> 
      satisfy_xy_supp_callback 
          = [&xy_supp_counter,
             &ctx](const GUNDAM::Match<Pattern, DataGraph>& match) -> bool {
      if (ctx.xy_supp_limit_ == -1) {
        // x_supp_limit is not specified 
        // continue matching
        return true;
      }
      assert(xy_supp_counter < ctx.xy_supp_limit_);
      xy_supp_counter++;
      if (xy_supp_counter == ctx.xy_supp_limit_) {
    ...
    messages.ParallelProcess<std::string>(
        ...
        1, [&ctx, 
          ...
          if (res_info_finish.first == kInfoFinishPrefix.end()) {
            ...
            ctx.final_result_.first  += std::stoi( supp_x_str);
            ctx.final_result_.second += std::stoi(supp_xy_str);
    ...
    if (receive_reset_parameter) {
      if (ctx.CurrentGarCollectMatchResult()) {
        assert(ctx.CurrentRhsMatchFile() != "");
        std::ofstream rhs_match_file(ctx.CurrentRhsMatchFile());
        ctx.match_set_.Serialize(rhs_match_file);
      }
      ctx.ResetParameter();
      ctx.ToNextGar();
      if (!ctx.HasGarToProcess()) {
        if (ctx.fid_ == kTimmerFragID
         && ctx.GarSetSize() > 1) {
          // multi gar
          if (ctx.time_log_file_.is_open()) {
    ...
    if (receive_info_process) {
      ...
      timer_next("distributed gar match");
      if (!ctx.work_load_balance_) {
    ...
    if (receive_info_finish) {
      ...
      if (ctx.time_log_file_.is_open()) {
        ctx.time_log_file_ << ctx.CurrentGarName() 
                           << "<x_supp, xy_supp>: <" 
                           << ctx.final_result_.first << "," 
                           << ctx.final_result_.second << ">" 
                           << std::endl;
        ctx.time_log_file_ << timer_now() << std::endl;
      }
      // distributed match end
      util::Output("<x_supp, xy_supp>: <" + std::to_string(ctx.final_result_.first) 
                                    + "," + std::to_string(ctx.final_result_.second)
                                    + ">");
      // has gar to process, info all worker to reset the
    ...
    if (receive_info_process || receive_info_status) {
      ...
      if (all_empty) {
        ...
        util::Output("<x_supp, xy_supp>: <" + std::to_string(ctx.final_result_.first) 
                                      + "," + std::to_string(ctx.final_result_.second)
                                      + ">");
        // has gar to process, begin next round
        auto& channel_0 = messages.Channels()[0];
        for (int dst_fid = 0; dst_fid < ctx.frag_num_; dst_fid++) {
          std::string msg(kResetParameterPrefix);
          channel_0.SendToFragment(dst_fid, std::move(msg));
    ...
    if (!ctx.work_load_balance_) {
      ...
      if (ctx.is_incremental_) {
        result = ctx.CurrentGarCollectMatchResult()?
                  gar::IncrementalGarSupp(ctx.CurrentGar(),
                                           ctx.data_graph_,
                                           ctx.data_graph_pivot_range_,
                                           satisfy_x_supp_collect_output_callback,
                                           satisfy_xy_supp_callback,
      ...
      std::string msg(kInfoFinishPrefix);
      auto& channel_0 = messages.Channels()[0];
      channel_0.SendToFragment(kTimmerFragID, msg + " " + std::to_string(result.first)
    ...
    while (!ctx.data_graph_pivot_range_.empty()) {
      ...
      std::pair<size_t, size_t> 
        temp_result = ctx.CurrentGarCollectMatchResult()?
                      gar::GarSupp(
                        ctx.CurrentGar(), 
                        ctx.data_graph_,
                        ctx.candidate_set_,
...
#endif  // EXAMPLES_ANALYTICAL_APPS_GAR_MATCH_GAR_MATCH_H_