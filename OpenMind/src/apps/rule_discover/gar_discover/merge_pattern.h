...
inline void MergeExpandedPatterns(
  ...
  for (size_t merged_expand_node_id_vector_set_idx = 0;
    ...
    // ##############################################################
    // ##  does not clear the new_vertexes_id and lhs_literals(),  ##
    // ##  would be used to expand lhs literal                     ##
    // ##----------------------------------------------------------##
    // ##  :  ## optimized me! ##                            ##
    // ##     merge the lhs literals in the similar way as the     ##
    // ##     rhs literals                                         ##
    // ##############################################################
    // added_node.lhs_literals().clear();
    // added_node.new_vertexes_id().clear();
    ...
    if (restriction.horn_rule()) {
      // #########################################
      // ##  horn rules consider only literals  ##
      // ##  connect two end vertexes           ##
      // #########################################
      // : optimize me 
      //    does not generate the rhs literals at all
      //    if it is a horn rule
    ...
    util::Info("literals_between_two_end_points: ");
    for (const auto& rhs_literal : literals_between_two_end_points) {
      std::string rhs_literal_str;
      rhs_literal_str << rhs_literal;
      util::Info("\t" + rhs_literal_str);
    }
    std::string pattern_str;
    pattern_str << added_node.pattern();
    util::Info("pattern_str: " + pattern_str);
    for (const auto& rhs_literal : added_node.const_rhs_literals()) {
      std::string rhs_literal_str;
      rhs_literal_str << rhs_literal;
      util::Info("\t" + rhs_literal_str);
