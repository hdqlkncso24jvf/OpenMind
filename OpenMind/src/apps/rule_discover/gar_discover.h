...
namespace grape
{
  ...
  private:
    ...
    using LiteralStandAloneInfoType = gar::LiteralStandAloneInfo<GraphPatternType,
                                                                 DataGraphType>;
  ...
  public:
               ...
               message_manager_t &messages)
    {
      ...
      {
        YAML::Node specified_edge_type_set_config = config["SpecifiedEdgeTypeSet"];
        for (int i = 0; i < specified_edge_type_set_config.size(); ++i)
        {
          YAML::Node specified_edge_type_config = specified_edge_type_set_config[i];
          ...
          auto [specified_edge_type_set_it,
                specified_edge_type_set_ret] = specified_edge_type_set.emplace(edge_type);
          if (!specified_edge_type_set_ret)
          {
            // add failed
            util::Error(std::string("specified duplicate edge type: <src_label, edge_label, dst_label>: ") + "<" + std::to_string(std::get<0>(edge_type)) + "," + std::to_string(std::get<1>(edge_type)) + "," + std::to_string(std::get<2>(edge_type)) + ">");
            return;
          }
        }
        ...
        util::Info("# Specified edge type number: " + std::to_string(specified_edge_type_set.size()) + " #");
      }
      ...
      {
        YAML::Node specified_edge_label_set_config = config["SpecifiedEdgeLabelSet"];
        for (int i = 0; i < specified_edge_label_set_config.size(); ++i)
        {
          EdgeLabelType specified_edge_label = specified_edge_label_set_config[i].as<EdgeLabelType>();
          auto [specified_edge_label_set_it,
                specified_edge_label_set_ret] = specified_edge_label_set.emplace(specified_edge_label);
          if (!specified_edge_label_set_ret)
          {
            util::Error("duplicate edge label: \t" + std::to_string(specified_edge_label));
            return;
          }
        }
      ...
      if (config["ConstantFreqBound"])
      {
        ...
        if (ctx.restriction_.constant_freq_bound() < 0.0 || ctx.restriction_.constant_freq_bound() > 1.0)
        {
      ...
      for (const auto &[edge_label,
                        edge_label_counter] : ctx.graph_basic_statistics_
                                                  .edge_label_counter())
      {
        if (edge_label < 0)
        {
      ...
      EdgeLabelType edge_label_offset = max_edge_label - minimal_negtive_edge_label;

      for (const auto &[edge_label, edge_label_counter] : ctx.graph_basic_statistics_
                                                              .edge_label_counter())
      {
        assert(ctx.ml_to_normal_edge_label_.find(edge_label + edge_label_offset) == ctx.ml_to_normal_edge_label_.end());
        assert(ctx.normal_to_ml_edge_label_.find(edge_label) == ctx.normal_to_ml_edge_label_.end());
      ...
      for (const auto &[edge_type, edge_type_counter] : ctx.graph_basic_statistics_
                                                            .edge_type_counter())
      {
        auto [ml_edge_type_set_it,
              ml_edge_type_set_ret] = ctx.ml_edge_type_set_.emplace(std::get<0>(edge_type), // src_label
      ...
      if (config["ConfidenceBound"])
      {
        ...
        if (ctx.restriction_.confidence_bound() < 0.0 || ctx.restriction_.confidence_bound() > 1.0)
        {
      ...
      {
        YAML::Node specified_rhs_literal_set_config = config["SpecifiedRhsLiteralSet"];
        for (int specified_rhs_literal_set_config_idx = 0;
             specified_rhs_literal_set_config_idx < specified_rhs_literal_set_config.size();
             specified_rhs_literal_set_config_idx++)
        {
          const auto &specified_rhs_literal_config = specified_rhs_literal_set_config[specified_rhs_literal_set_config_idx];
          auto [stand_alone_literal_info,
                stand_alone_literal_info_ret] = util::LiteralStandAloneInfoFromYaml<GraphPatternType,
                                                                                    DataGraphType>(specified_rhs_literal_config);
          if (!stand_alone_literal_info_ret)
          {
            util::Error("the " + std::to_string(specified_rhs_literal_set_config_idx) + "'th literal is illegal");
            return;
          }
          ...
          const std::string kLiteralSuppFile = specified_rhs_literal_config["SuppFile"].as<std::string>();

          std::ifstream literal_supp_file(kLiteralSuppFile);

          if (!literal_supp_file.good())
          {
            util::Error("literal_supp_file of the " + std::to_string(specified_rhs_literal_set_config_idx) + "'th literal: " + kLiteralSuppFile + " is not good! ");
            return;
          }

          std::vector<
              std::vector<typename GUNDAM::VertexHandle<DataGraphType>::type>>
              literal_supp_set;
          ...
          while (literal_supp_file)
          {
            ...
            {
              if ((kLiteralVertexNum + 1) != support_id_set.size() || (support_id_set[kLiteralVertexNum] != 0 && support_id_set[kLiteralVertexNum] != 1))
              {
                util::Error("support set miss match in line: " + std::to_string(literal_supp_set.size()) + " of the " + std::to_string(specified_rhs_literal_set_config_idx) + "'th literal");
                return;
              }
            }
            if (kLiteralVertexNum + 1 == support_id_set.size())
            {
              assert(support_id_set[kLiteralVertexNum] == 0 || support_id_set[kLiteralVertexNum] == 1);
            ...
            for (const auto &vertex_id : support_id_set)
            {
              auto vertex_handle = ctx.data_graph_set_.at(0)
                                       .data_graph()
                                       .FindVertex(vertex_id);
              if (!vertex_handle)
              {
                util::Error("vertex: " + std::to_string(vertex_id) + " cannot be found in data graph!");
                return;
              }
          ...
          {
            util::Error("Specified empty support set for the " + std::to_string(specified_rhs_literal_set_config_idx) + "'th literal");
            return;
          }
        ...
        util::Info("specified_rhs_literal_set size: " + std::to_string(ctx.restriction_.specified_rhs_literal_set().size()));
      }
      ...
      if (config["LiteralTypes"])
      {
        ...
        {
          const std::string literal_type = literal_types_config[i].as<std::string>();
      ...
      if (config["LhsLiteralTypes"])
      {
        ...
        {
          const std::string lhs_literal_type = lhs_literal_types_config[i].as<std::string>();
      ...
      if (config["Pattern"])
      {
        ...
        if (ctx.restriction_.pattern_is_star())
        {
          ...
          if (star_config["PathNumLimit"])
          {
            ...
            if (kPathNumLimit < 1)
            {
              util::Error("illegal PathNumLimit for gcr: " + std::to_string(kPathNumLimit));
              return;
            }
          ...
          if (star_config["PathLengthLimit"])
          {
            ...
            {
              util::Error("illegal PathLengthLimit for gcr: " + std::to_string(kPathLengthLimit));
              return;
            }
      ...
      if (config["Rule"])
      {
        ...
        const std::string rule_type = rule_config["Type"].as<std::string>();
        ...
        if (ctx.restriction_.gcr())
        {
          ...
          if (rule_config["Restrictions"])
          {
            ...
            {
              const std::string restriction = restrictions_config[i].as<std::string>();
      ...
      assert(ctx.restriction_.gar() || ctx.restriction_.gcr() || ctx.restriction_.horn_rule());
      ...
      if (ctx.fid_ == kExpandPatternFragID && ctx.log_file_.is_open())
      {
        // export log
      ...
      if (ctx.fid_ != kExpandPatternFragID)
      {
        ...
        msg += " " + std::to_string(ctx.fid_) + " " + std::to_string(omp_get_num_procs());
      ...
      std::set<std::pair<VertexLabelType,
                         VertexLabelType>>
          considered_label_pair_set;
      for (const auto &[vertex_label,
                        vertex_label_counter] : ctx.graph_basic_statistics_
                                                    .vertex_label_counter())
      {
        ...
        for (const auto &[other_vertex_label,
                          other_vertex_label_counter] : ctx.graph_basic_statistics_
                                                            .vertex_label_counter())
        {
          auto [considered_label_pair_set_it,
                considered_label_pair_set_ret] = considered_label_pair_set.emplace(std::min(vertex_label, other_vertex_label),
      ...
      {
        assert(ctx.root_pattern_max_edge_id_ == 0 || ctx.root_pattern_max_edge_id_ == root_pattern.CountEdge());
        ...
        auto &expand_tree_node = ctx.expand_level_.AddExpandTreeNode(
            pattern_counter++,
            root_pattern,
        ...
        util::Info("root_pattern_str: " + root_pattern_str);

        util::Info("possible_literals.size(): " + std::to_string(possible_literals.size()));
                 ...
                 message_manager_t &messages)
    {
      ...
      using ExpandTreeNodeType = ExpandTreeNode<GraphPatternType,
                                                DataGraphType>;

      using ExpandTreeLevelType = ExpandTreeLevel<GraphPatternType,
                                                  DataGraphType>;

      using GenerateTreeNodeType = GenerateTreeNode<GraphPatternType,
                                                    DataGraphType>;

      using GenerateTreeLevelType = GenerateTreeLevel<GraphPatternType,
                                                      DataGraphType>;
      ...
      assert(ctx.fid_ == kExpandPatternFragID || ctx.expand_level_.empty());

      util::Info("omp_get_num_procs: " + std::to_string(omp_get_num_procs()));
      ...
      if (ctx.fid_ == kExpandPatternFragID)
      {
        ...
        if (receive_update_expand_node || receive_info_process)
        {
          // ##################################################
          ...
          util::Output(" current round: " + std::to_string(ctx.current_round_) + "  expand round: " + std::to_string(ctx.expand_round_));
          ...
          util::Output(" round: " + std::to_string(ctx.current_round_) + " generated : " + std::to_string(ctx.expand_level_.size()) + " pattern.");
          ...
          if (ctx.current_round_ > 0)
          {
            ...
            util::Output("expand end!, total pattern number: " + std::to_string(next_expand_level.size()));
            ...
            util::Output("unique time is " + std::to_string(unique_time) + "s");
            util::Output("uniqued pattern number: " + std::to_string(next_expand_level.size()));
               ...
               expand_level_idx++)
          {
            ...
            if (next_expand_tree_node.rhs_literals().empty())
            {
              ...
              assert(std::get<1>(temp_message) == std::get<1>(temp_temp_message));
              assert(std::get<2>(temp_message) == std::get<2>(temp_temp_message));
          ...
          for (int dst_fid = 0; dst_fid < ctx.frag_num_; dst_fid++)
          {
            ...
            assert(dst_fid != kExpandPatternFragID || msg_to_workers[dst_fid] == "");
            channel_0.SendToFragment(dst_fid, std::move(msg) + std::move(msg_to_workers[dst_fid]));
          }
        ...
        if (receive_update_pattern)
        {
               ...
               expand_node_idx++)
          {
            ...
            auto &satisfy_supp_bound_ref = satisfy_supp_bound.emplace_back(std::vector<bool>());
            ...
            assert(satisfy_supp_bound.back().size() == satisfy_supp_bound_ref.size());
            assert(satisfy_supp_bound.back().size() == ctx.pattern_match_count_[expand_node_idx].size());
          }
          // whether each pattern satisfy the support bound on each
          // data graph has been generated
          assert(satisfy_supp_bound.size() == next_expand_level.size());
          // index the expand_nodes
          next_expand_level.ReorderNodeId();
          ctx.expand_level_.Swap(next_expand_level);
          assert(satisfy_supp_bound.size() == ctx.expand_level_.size());
#ifndef NDEBUG
          ...
          assert(worker_rhs_number.size() == ctx.process_num_.size());
               ...
               expand_node_idx++)
          {
            ...
            for (const bool &satisfy_supp_bound_on_data_graph : satisfy_supp_bound[expand_node_idx])
            {
            ...
            std::sort(worker_rhs_number.begin(),
                      ...
                      {
                        return ((double)std::get<0>(a)) / ((double)std::get<1>(a)) < ((double)std::get<0>(b)) / ((double)std::get<1>(b));
                      });
            ...
            {
              util::Debug("#" + std::to_string(rhs_number) + "," + std::to_string(process_number) + "," + std::to_string(worker_id) + "#");
            }
#endif // NDEBUG
            ...
            util::Debug("processing expand node: " + std::to_string(expand_node.id()) + " to worker: " + std::to_string(to_worker));
      ...
      {
        assert(receive_deliver_expand_node || receive_deliver_pattern);
      ...
      if (receive_deliver_pattern)
      {
        ...
        util::Info("total patterns to evalute: " + std::to_string(total_patterns_to_evalute));
...
#pragma omp parallel for schedule(dynamic)
        ...
        {
          util::Debug("* Evaluate match count: number of threads in the team - " + std::to_string(omp_get_num_threads()) + "\n");

          auto kDeliveredPatternId = (ctx.fid_ == kExpandPatternFragID) ? ctx.process_pattern_id_set_[i].first
                                                                        : std::get<1>(delivered_patterns[i]);

          util::Debug("kDeliveredPatternId: " + std::to_string(kDeliveredPatternId));

          // the pattern for this worker to match
          auto &delivered_pattern = (ctx.fid_ == kExpandPatternFragID) ? ctx.expand_level_.node(kDeliveredPatternId).pattern()
                                                                       : std::get<0>(delivered_patterns[i]);

          // whether to only verify whether it is legal
          const bool kDeliveredPatternOnlyVerifyIsLegal = (ctx.fid_ == kExpandPatternFragID) ? ctx.process_pattern_id_set_[i].second
                                                                                             : std::get<2>(delivered_patterns[i]);

          util::Debug("kDeliveredPatternOnlyVerifyIsLegal: " + std::to_string(kDeliveredPatternOnlyVerifyIsLegal));

#ifndef NDEBUG
          if (ctx.fid_ == kExpandPatternFragID)
          {
            // if only verify whether it is legal, then it should not have legal rhs literal
            assert((kDeliveredPatternOnlyVerifyIsLegal && ctx.expand_level_.node(kDeliveredPatternId).rhs_literals().empty()) || (!kDeliveredPatternOnlyVerifyIsLegal && !ctx.expand_level_.node(kDeliveredPatternId).rhs_literals().empty()));
          }
#endif // NDEBUG

          // if only needs to verify whether it is legal, then the kMatchCountLimit
          // would be set to 1
          const size_t kMatchCountLimit = kDeliveredPatternOnlyVerifyIsLegal ? 1 : ctx.support_bound_;
               ...
               data_graph_idx++)
          {
            ...
            if (ctx.restriction_.gar() || ctx.restriction_.horn_rule())
            {
              // ##############################################
            ...
            else
            {
              ...
              const bool kInitCandidateSetSucc = GUNDAM::_dp_iso_using_match ::InitCandidateSet<GUNDAM::MatchSemantics::kHomomorphism>(
                  delivered_pattern,
                  data_graph,
                  candidate_set);
              if (kInitCandidateSetSucc)
              {
                if (ctx.restriction_.central_to_central())
                {
                  ...
                  for (const auto &connected_component : connected_components)
                  {
                    ...
                    for (const auto &vertex_handle : central_vertex_handle_set)
                    {
                      auto central_candidate_set_it = candidate_set.find(vertex_handle);
                      assert(central_candidate_set_it != candidate_set.end());
                      ...
                      assert(!ctx.data_graph_set_[data_graph_idx]
                                  ...
                                  .empty());
                      const auto &data_graph_centrel_vertex_set = ctx.data_graph_set_[data_graph_idx]
                                                                      .centrel_vertex_set(vertex_handle->label());
                ...
                const bool kRefineCandidateSetSucc = GUNDAM::_dp_iso_using_match ::RefineCandidateSet(delivered_pattern,
                                                                                                      data_graph,
                                                                                                      candidate_set);
                if (kRefineCandidateSetSucc)
                {
                  std::vector<GraphPatternType> connected_components = GUNDAM::ConnectedComponent(delivered_pattern);
                  assert(connected_components.size() == 2);
                  // ##########################################################
                       ...
                       cc_idx++)
                  {
                    ...
                    {
                      assert(delivered_pattern.FindVertex(vertex_it->id()));
                      assert(candidate_set.find(delivered_pattern.FindVertex(vertex_it->id())) != candidate_set.end());
                      ...
                      minimal_candidate_size[cc_idx] = minimal_candidate_size[cc_idx] < candidate_set[delivered_pattern.FindVertex(vertex_it->id())].size() ? minimal_candidate_size[cc_idx] : candidate_set[delivered_pattern.FindVertex(vertex_it->id())].size();
                    }
                  }
                  ...
                  match_count = match_count < kMatchCountLimit ? match_count : kMatchCountLimit;
                }
#ifndef NDEBUG
                else
                {
            ...
            util::Debug("< kMatchCountLimit: " + std::to_string(kMatchCountLimit) + ", match_count: " + std::to_string(match_count) + " >");
        ...
        if (ctx.fid_ == kExpandPatternFragID)
        {
          ...
          {
            assert(pattern_id >= 0 && pattern_id < ctx.pattern_match_count_.size());
            assert(graph_id >= 0 && graph_id < ctx.pattern_match_count_[pattern_id].size());
            ctx.pattern_match_count_[pattern_id][graph_id] = pattern_supp;
#ifndef NDEBUG
            if (ctx.support_bound_ != 1 && ctx.support_bound_ == pattern_supp)
            {
        ...
        for (const auto &[pattern_id,
                          ...
                          graph_id] : patterns_support)
        {
          util::Debug(" pattern_id: " + std::to_string(pattern_id) + " pattern_supp: " + std::to_string(pattern_supp) + " graph_id: " + std::to_string(graph_id));
          if (pattern_supp == 0)
          {
            // there is no need to pass such info
            continue;
          }
          msg = std::move(msg) + " " + std::to_string(pattern_id) + " " + std::to_string(pattern_supp) + " " + std::to_string(graph_id);
        }
        channel_0.SendToFragment(kExpandPatternFragID, msg);
        return;
      }
      ...
      const SubstructureLevelType kCurrentPatternEdgeSize = ctx.expand_level_.node(ctx.process_expand_node_id_set_.front())
                                                                .const_pattern()
                                                                .CountEdge();
      ...
      util::Info("number of nodes for horizontal spawning: " + std::to_string(ctx.process_expand_node_id_set_.size()));
...
#pragma omp parallel for schedule(dynamic)
           ...
           process_expand_node_idx++)
      {
        ...
        if ((process_expand_node_idx != 0) && (kTenPercent != 0) && (process_expand_node_idx % kTenPercent == 0))
        {
          util::Info("horizontal spawning completed: " + std::to_string(process_expand_node_idx / kTenPercent) + "0% ");
        }
        util::Debug("# Evaluate literal support: number of threads in the team - " + std::to_string(omp_get_num_threads()) + "\n");
        // get the reference of the current processing expand node
        auto &current_expand_node = ctx.expand_level_.node(
            ctx.process_expand_node_id_set_[process_expand_node_idx]);
        const auto kCurrentExpandNodeId = current_expand_node.id();
        ...
        const auto &current_expand_pattern = current_expand_node.const_pattern();
             ...
             data_graph_idx++)
        {
          ...
          if (ctx.restriction_.gcr())
          {
            ...
            init_candidate_set_succ = GUNDAM::_dp_iso_using_match ::InitCandidateSet<GUNDAM::MatchSemantics::kHomomorphism>(
          ...
          else
          {
            ...
            init_candidate_set_succ = GUNDAM::_dp_iso_using_match ::InitCandidateSet<GUNDAM::MatchSemantics::kIsomorphism>(
                current_generate_node.pattern(),
                ctx.data_graph_set_[data_graph_idx].data_graph(),
                candidate_set);
          }
        ...
        util::Debug("rhs_literal_info_set.size(): " + std::to_string(rhs_literal_info_set.size()));
        ...
        {
          // ############################################################
          // ##  expand literl for lhs that are not considered before  ##
          // ##  :  ## optimized me! ##                          ##
          // ##     merge the lhs literals in the similar way as the   ##
          // ##     rhs literals                                       ##
          // ############################################################

          lhs_literal_info_not_in_rhs_set.swap(current_expand_node.lhs_literals());

          util::Debug("lhs literal size before expand: " + std::to_string(lhs_literal_info_not_in_rhs_set.size()));
          ...
          util::Debug("lhs literal size after expand: " + std::to_string(lhs_literal_info_not_in_rhs_set.size()));
          ...
          util::Debug("lhs literal size after unique: " + std::to_string(lhs_literal_info_not_in_rhs_set.size()));

          for (auto lhs_literal_it = lhs_literal_info_not_in_rhs_set.begin();
               lhs_literal_it != lhs_literal_info_not_in_rhs_set.end();)
          {
            if (lhs_literal_it->literal_type() == gar::LiteralType::kEdgeLiteral || !ctx.restriction_.LiteralTypeConsideredInLhs(lhs_literal_it->literal_type()))
            {
          ...
          util::Debug("lhs literal size after remove literal in rhs: " + std::to_string(lhs_literal_info_not_in_rhs_set.size()));
        }

        // they should be empty now
        assert(current_expand_node.RhsLiteralsBegin() == current_expand_node.RhsLiteralsEnd());
        assert(current_expand_node.LhsLiteralsBegin() == current_expand_node.LhsLiteralsEnd());
             ...
             rhs_literal_idx++)
        {
          ...
          for (auto &[existed_gar,
                      existed_gar_rhs_literal_idx] : existed_gars)
          {
        ...
        util::Debug("rhs literal number before remove automorphism: " + std::to_string(rhs_literal_info_set.size()));
        ...
        rhs_literal_id_set_to_evaluate.erase(
            std::unique(rhs_literal_id_set_to_evaluate.begin(),
                        rhs_literal_id_set_to_evaluate.end()),
            rhs_literal_id_set_to_evaluate.end());
        util::Debug("rhs literal number after remove automorphism: " + std::to_string(rhs_literal_id_set_to_evaluate.size()));
             ...
             rhs_literal_id_set_to_evaluate_idx++)
        {
               ...
               data_graph_idx++)
          {
            ...
            const auto &candidate_set = candidate_set_for_data_graph_set.at(data_graph_idx);
            ...
            const auto &candidate_set_removed_nec = candidate_set_removed_nec_for_data_graph_set.at(data_graph_idx);
            ...
            auto [temp_supp, temp_conf] = HasMatch(current_generate_node.pattern(),
                                                   literal_set,
                                                   literal_info,
                                                   data_graph,
                                                   ctx.normal_to_ml_edge_label_,
            ...
            assert(temp_supp >= 0 && (temp_supp <= ctx.support_bound_ || ctx.restriction_.specified_confidence_bound()));
            ...
            assert(ctx.restriction_.specified_confidence_bound() || supp <= ctx.support_bound_);
          ...
          assert(conf >= ctx.restriction_.confidence_bound() || conf == 0);
        ...
        {
          size_t group_id = rhs_disjoint_set.Find(rhs_literal_info_idx);
          const auto &rhs_literal_info = rhs_literal_info_set[rhs_literal_info_idx];
          if (literal_can_be_considered_in_rhs[group_id])
          {
            assert(rhs_literal_info.literal_type() == gar::LiteralType::kEdgeLiteral || literal_can_be_considered_in_lhs[group_id]);
            if (ctx.j_ > 1)
            {
              // is legal, add to Lhs
            ...
            if (ctx.restriction_.specified_confidence_bound() && ctx.restriction_.confidence_bound() <= rhs_literal_conf[group_id])
            {
        ...
        if (ctx.j_ > 1)
        {
          ...
          for (size_t lhs_literal_idx = 0;
               ...
               lhs_literal_idx++)
          {
            const auto &literal_info = lhs_literal_info_not_in_rhs_set[lhs_literal_idx];
            const std::vector<DataGraphLiteralType> literal_set = {literal_info};
            GarType new_gar(current_generate_node.const_pattern());
            new_gar.AddY(literal_info);
            bool has_automorphism = false;
          ...
          util::Debug("lhs literal number before remove automorphism: " + std::to_string(lhs_literal_info_not_in_rhs_set.size()));
          ...
          util::Debug("lhs literal number after remove automorphism: " + std::to_string(lhs_literal_id_set_to_evaluate.size()));
          ...
          {
            size_t group_id = lhs_disjoint_set.Find(lhs_literal_idx);
            const auto &lhs_literal_info = lhs_literal_info_not_in_rhs_set[lhs_literal_idx];
        ...
        util::Debug("current_expand_node.const_rhs_literals().size(): " + std::to_string(current_expand_node.const_rhs_literals().size()));
        ...
        assert(current_expand_node_rhs_literal_conf.size() == current_expand_node_rhs_literal.size());

        // enumerate the remain literal as the root of literal trees
        current_generate_node.InitLiteralTrees(current_expand_node_rhs_literal,
             ...
             literal_idx++)
        {
          ...
          if (ctx.restriction_.specified_confidence_bound())
          {
            if (current_expand_node_rhs_literal_conf[literal_idx] >= ctx.restriction_.confidence_bound())
            {
              continue;
            }
          }
        ...
        for (const auto &rhs_literal : rhs_literal_info_satisfy_supp_bound_set_duplicated)
        {
          current_expand_node.AddRhsLiteral(rhs_literal);
        }

        if (current_generate_node.LiteralTreesBegin() == current_generate_node.LiteralTreesEnd())
        {
             ...
             literal_tree_it++)
        {
          ...
          LiteralTreePtr literal_tree_root = literal_tree.VertexBegin();
          ...
          if (ctx.restriction_.specified_confidence_bound())
          {
            const float kConfidence = literal_tree_root
          ...
          DataGraphLiteralType y_literal = literal_tree_root
          ...
          while (!literal_stack.empty())
          {
            auto [current_literal_handle,
                  current_literal_is_first_considered] = literal_stack.top();
            literal_stack.pop();
            ...
            if (!current_literal_is_first_considered)
            {
              ...
              if (existed_literals.size() != current_literal_handle->label())
              {
                ...
                util::Error("# existed_literals.size(): " + std::to_string(existed_literals.size()) + "# current_literal_handle->label(): " + std::to_string(current_literal_handle->label()));
              }
            ...
            assert(!ctx.restriction_.specified_confidence_bound() || (current_literal_handle
            ...
            for (const auto &expandable_literal : expandable_literals)
            {
              ...
              assert(expandable_literal.literal_type() != gar::LiteralType::kEdgeLiteral);
              ...
              if (ctx.restriction_.specified_confidence_bound() && satisfy_conf_bound)
              {
      ...
      if (ctx.fid_ != kExpandPatternFragID)
      {
        // the kExpandPatternFragID does not need to transform data through the
        // MPI method,
        assert(ctx.expand_level_.size() == ctx.process_expand_node_id_set_.size());
