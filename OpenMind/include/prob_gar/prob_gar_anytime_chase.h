...
int ProbGARAnytimeChase(
  ...
  while (time_limit < 0 // no time limit
    ...
    if (!atom_set_modified_in_last_round.empty()) {
      util::Debug(": atom_set_modified_in_last_round.size(): "
      ...
      std::function<bool(const MatchType&)> 
        ...
        if (edge_handle) {
          ...
          if (edge_handle->id() <= max_data_graph_edge_id) {
            ...
            util::Debug("#### : is original edge ####");
            return true;
          }
          util::Debug("#### : is not original edge ####");
          ...
          // #endif // NDEBUG
          util::Debug("######################################");
          util::Debug("#### : has not satisfy edge ####");

          util::Debug("#### : match.MapTo(y_literal_info.x_id())->id(): "
                   + std::to_string(match.MapTo(y_literal_info.x_id())->id()));

          util::Debug("#### : match.MapTo(y_literal_info.y_id())->id(): "
                   + std::to_string(match.MapTo(y_literal_info.y_id())->id()));

          util::Debug("#### : y_literal_info.edge_label(): "
                   + std::to_string(y_literal_info.edge_label()));
          ...
          util::Debug("******************************************");
          util::Debug("*** : atom_to_update.edge_id() before: "
          ...
          util::Debug("******************************************");
          util::Debug("*** : atom_to_update.edge_id() after: "
                  + std::to_string(atom_to_update.edge_id()));
          util::Debug("******************************************");
          assert(!data_graph.FindEdge(atom_to_update.edge_id()));
        }
        ...
        util::Debug("##########################################");
        util::Debug("### : atom_to_update.edge_id(): "
                + std::to_string(atom_to_update.edge_id()));
        util::Debug("##########################################");
    ...
    util::Debug(": current_round_tcp_set.size(): "
    ...
    for (const auto& [current_atom, tcp] : current_round_tcp_set) {
      ...
      if (modified || tcp_set_ret) {
        // util::Debug("*********************************************");
        // util::Debug("*  : modified!  atom: " + current_atom.ToString());
        // util::Debug("*********************************************");
        ...
        // util::Debug("*********************************************");
        // util::Debug("*  : not modified!  atom: " + current_atom.ToString());
        // util::Debug("*********************************************");
