                       ...
                       std::vector<PathGraphType> &path_set)
  {
    ...
    if (star.CountVertex() == 1)
    {
           ...
           path_it++)
      {
        ...
        if (path_src_handle->label() == vertex_it->label() || path_dst_handle->label() == vertex_it->label())
        {
          return true;
        }
      }
    ...
    if (!central_vertex_handle)
    {
           ...
           !c_vertex_it.IsDone(); c_vertex_it++)
      {
        ...
        if (c_vertex_handle == pattern_dst_handle)
          continue; // already been checked when v_vertex_it == pattern_src_handle
             ...
             path_it++)
        {
          ...
          std::function<bool(const Match<PathGraphType, StarGraphType> &)>
              match_callback = [&unmatched_ends, &path_target_handle](const Match<PathGraphType, StarGraphType> &match_state) -> bool
          {
            ...
            if (unmatched_ends.find(match_state.MapTo(path_target_handle)->id()) != unmatched_ends.end())
            {
    ...
    else
    {
           ...
           path_it++)
      {
        ...
        std::function<bool(const Match<PathGraphType, StarGraphType> &)>
            match_callback = [&unmatched_ends, &path_target_handle](const Match<PathGraphType, StarGraphType> &match) -> bool
        {
          // matches all endpoints until no unmatched endpoints exist
          if (unmatched_ends.find((match.MapTo(path_target_handle))->id()) != unmatched_ends.end())
          {
