  ...
  namespace _dp_iso_using_match
  {
                        ...
                        const typename VertexHandle<GraphType>::type &dst_vertex_handle)
    {
      ...
      else
      {
        ...
        for (auto edge_it = vertex_handle->OutEdgeBegin();
             ...
             edge_it++)
        {
          if (edge_it->label() == edge_label && edge_it->dst_handle() == dst_vertex_handle)
          {
            counter++;
          }
        }
                                ...
                                const TargetVertexHandle &target_vertex_handle)
    {
      if (query_vertex_handle->CountInEdge() > target_vertex_handle->CountInEdge())
        return false;
      if (query_vertex_handle->CountOutEdge() > target_vertex_handle->CountOutEdge())
        return false;
      if (query_vertex_handle->CountInVertex() > target_vertex_handle->CountInVertex())
        return false;
      if (query_vertex_handle->CountOutVertex() > target_vertex_handle->CountOutVertex())
        return false;
      return true;
    }
        ...
        const typename VertexHandle<TargetGraph>::type &target_vertex_handle)
    {
           ...
           edge_label_it++)
      {
        auto [used_edge_label_it,
              used_edge_label_ret] = used_edge_label.emplace(edge_label_it->label());
                 ...
                 std::vector<typename VertexHandle<TargetGraph>::type>> &candidate_set)
    {
      using QueryVertexHandle = typename VertexHandle<QueryGraph>::type;
      using TargetVertexHandle = typename VertexHandle<TargetGraph>::type;
      using CandidateContainer = std::vector<typename VertexHandle<TargetGraph>::type>;
      static_assert(std::is_same_v<policy, execution::sequenced_policy> || std::is_same_v<policy, execution::parallel_policy>, "illegal policy");
      if constexpr (std::is_same_v<policy, execution::sequenced_policy>)
      {
        for (auto it = begin_it; it != end_it; it++)
        {
          ...
          auto candidate_set_it = candidate_set.find(query_vertex_handle);
...
#pragma omp parallel for schedule(dynamic)
    ...
    {
      using QueryVertexHandle = typename VertexHandle<QueryGraph>::type;
      for (auto edge_iter = (edge_direction == EdgeDirection::kIn ? query_vertex_handle->InEdgeBegin()
           ...
           edge_iter++)
      {
        ...
        if (!match_state.HasMap(query_opp_vertex_handle) && candidate_set.find(query_opp_vertex_handle) != candidate_set.end())
        {
          if constexpr (use_dag_order)
          {
            if (!parent.empty())
            {
              ...
              for (const auto &parent_vertex_handle : parent_it->second)
              {
                       ...
                       std::vector<typename VertexHandle<QueryGraph>::type>> &parent)
    {
      ...
      {
        assert(candidate_set.find(query_vertex_handle) != candidate_set.end());
        size_t candidate_count = candidate_set.find(query_vertex_handle)->second.size();
        ...
        const Match<QueryGraph, TargetGraph> &match_state)
    {
      ...
      if (query_vertex_handle->label() != target_vertex_handle->label()) /* [[unlikely]] */
      {
        return false;
      }
           ...
           query_edge_iter++)
      {
        ...
        TargetVertexHandle query_opp_match_vertex_handle = match_state.MapTo(query_opp_vertex_handle);
        ...
        if constexpr (GraphParameter<TargetGraph>::vertex_level_edge_label_index)
        {
               ...
               target_edge_iter++)
          {
            ...
            {
              if (used_edge.find(target_edge_handle) != used_edge.end())
              {
        ...
        else
        {
               ...
               target_edge_iter++)
          {
            ...
            if constexpr (match_semantics == MatchSemantics::kIsomorphism)
            {
              if (used_edge.find(target_edge_iter) != used_edge.end())
              {
                continue;
              }
            }
    ...
    {
      if (match_semantics == MatchSemantics::kIsomorphism && target_matched.find(target_vertex_handle) != target_matched.end())
      {
        return false;
      }
        ...
        const Match<QueryGraph, TargetGraph> &match_state)
    {
           ...
           edge_label_it++)
      {
        ...
        // 枚举label
        if (used_edge_label.find(edge_label_it->label()) != used_edge_label.end())
        {
        ...
        if constexpr (GraphParameter<TargetGraph>::vertex_level_edge_label_index)
        {
          ...
          for (auto edge_it = ((edge_direction == EdgeDirection::kIn)
               ...
               edge_it++)
          {
            TargetVertexHandle temp_target_handle = (edge_direction == EdgeDirection::kIn) ? edge_it->src_handle()
                                                                                           : edge_it->dst_handle();
            auto temp_target_handle_it = temp_adj_vertex.find(temp_target_handle->label());
             ...
             edge_it++)
        {
          ...
          if (used_vertex.find(temp_vertex_handle) != used_vertex.end())
          {
            // this vertex has already been used
            continue;
          }
          auto [used_vertex_it,
                used_vertex_ret] = used_vertex.emplace(temp_vertex_handle);
          assert(used_vertex_ret);
          if (candidate_set.find(temp_vertex_handle) == candidate_set.end())
          {
            // (?)
            continue;
          }
          if (temp_adj_vertex.find(temp_vertex_handle->label()) == temp_adj_vertex.end())
          {
                                  ...
                                  std::vector<typename VertexHandle<QueryGraph>::type>> &parent)
    {
      ...
      std::map<typename QueryGraph::VertexCounterType,
               std::vector<QueryVertexHandleType>>
          grouped_query_vertex_set;

      auto build_dag_callback = [&grouped_query_vertex_set](
           ...
           layer_it++)
      {
        ...
        for (const auto &vertex_handle : vertex_set)
        {
          ...
          auto [parent_it, parent_ret] = parent.emplace(vertex_handle, std::vector<QueryVertexHandleType>{vertex_handle});
          // should have been added successfully
          assert(parent_ret);
          auto &parent_vertex_set = parent_it->second;
          assert(parent_vertex_set.size() == 1 && *parent_vertex_set.begin() == vertex_handle);
          assert(parent.find(vertex_handle) != parent.end());
        }
        ...
        if (distance > 0)
        {
          ...
          const auto &previous_layer_vertex_set = previous_layer_it->second;
          for (const auto &vertex_handle : vertex_set)
          {
                 ...
                 out_edge_it++)
            {
              ...
              auto &opp_vertex_handle_parent = opp_vertex_handle_parent_it->second;
        ...
        std::set<typename VertexHandle<TargetGraph>::type> &target_matched)
    {
      ...
      assert(target_matched.find(target_vertex_handle) != target_matched.end());
#ifndef NDEBUG
      for (auto map_it = match_state.MapBegin();
           !map_it.IsDone();
           map_it++)
        ...
        double query_limit_time)
    {
      ...
      if (query_limit_time > 0 && query_limit_time < (std::time(NULL) - begin_time))
      {
      ...
      QueryVertexHandle next_query_vertex_handle = NextMatchVertex<use_dag_order>(candidate_set, match_state, parent);
      ...
      const auto &candidate_set_for_next_query_vertex = candidate_set.find(next_query_vertex_handle)->second;
      ...
      for (const auto &next_target_vertex_handle : candidate_set_for_next_query_vertex)
      {
        if constexpr (use_fail_set)
        {
          if (!current_state_fail_set.empty() && !std::binary_search(current_state_fail_set.begin(),
          ...
          if (match_semantics == MatchSemantics::kIsomorphism && !has_one_child_with_empty_fail_set && target_matched.find(next_target_vertex_handle) != target_matched.end())
          {
            // #######################################################
                 ...
                 map_it++)
            {
              ...
              assert(parent.find(next_query_vertex_handle) != parent.end());
              assert(parent.find(map_it->src_handle()) != parent.end());
    ...
    namespace _DAGDP
    {
                      ...
                      const std::set<typename VertexHandle<QueryGraph>::type> &src_vertex_set)
      {
        ...
        if (query_graph.CountVertex() < kTooBig)
        {
          ...
          while (used_vertex.size() < query_graph.CountVertex())
          {
            if (temp_src_vertex_set.empty())
            {
                   ...
                   query_graph_vertex_it++)
              {
                ...
                auto [temp_src_vertex_set_it,
                      temp_src_vertex_set_ret] = temp_src_vertex_set.emplace(query_graph_vertex_it);
            ...
            {
              auto [used_vertex_it,
                    used_vertex_ret] = used_vertex.emplace(src_vertex);
              assert(used_vertex_ret);
              bfs_queue.emplace(src_vertex);
              auto [in_degree_it,
                    in_degree_ret] = in_degree.emplace(src_vertex, 0);
      ...
      inline void GetMultiWayTopoSeq(QueryGraph &query_graph,
                                     TopoSeqContainer &multi_way_topo_seq,
                                     const size_t batch_size,
                                     const typename VertexHandle<QueryGraph>::type src_vertex = typename VertexHandle<QueryGraph>::type())
      {
        using QueryVertexHandle = typename VertexHandle<QueryGraph>::type;
        using QueryEdgeIDType = typename EdgeID<QueryGraph>::type;
                         ...
                         std::map<typename EdgeLabel<QueryGraph>::type, int>> &in_edge_count)
      {
        ...
        assert((out_edge_count.find(adj_used_vertex_handle) != out_edge_count.end()) || (in_edge_count.find(adj_used_vertex_handle) != in_edge_count.end()));
          ...
          const CandidateSetContainer &candidate_set)
      {
        ...
        for (auto edge_it = EdgeBegin<is_out_direction, QueryGraph>(
             ...
             edge_it++)
        {
          QueryVertexHandle adj_used_vertex_handle = is_out_direction ? edge_it->dst_handle()
                                                                      : edge_it->src_handle();
          if (used_vertex.find(adj_used_vertex_handle) == used_vertex.end())
          {
            // this vertex is not used, does not need to be checked
            continue;
          }
          assert(candidate_set.find(adj_used_vertex_handle) != candidate_set.end());
          auto &candidate_set_for_adj_used_vertex = candidate_set.find(adj_used_vertex_handle)->second;
          bool found_legal_vertex_adj_to_target_vertex_handle = false;
          if constexpr (GraphParameter<TargetGraph>::vertex_level_edge_label_index)
          {
            // vertex has vertex_handle->In/Out VertexBegin(edge_label)
            // method
            for (auto target_edge_it = is_out_direction ? target_vertex_handle->OutEdgeBegin(edge_it->label())
                                                        : target_vertex_handle->InEdgeBegin(edge_it->label());
                 !target_edge_it.IsDone();
                 target_edge_it++)
            {
              TargetVertexHandle temp_target_vertex_handle = is_out_direction ? target_edge_it->dst_handle()
              ...
              if (it1 != candidate_set_for_adj_used_vertex.end() && (*it1) == temp_target_vertex_handle)
              {
          ...
          else
          {
                 ...
                 !target_edge_it.IsDone(); target_edge_it++)
            {
              ...
              TargetVertexHandle temp_target_vertex_handle = is_out_direction ? target_edge_it->dst_handle()  // is out edge
        ...
        TargetVertexContainerType &candidate_set_of_vertex_handle)
    {
      ...
      std::map<QueryVertexHandle,
               std::map<QueryEdgeLabelType, int>>
          in_edge_count,
          out_edge_count;
      ...
      for (const auto &candidate_dst_handle : candidate_set_of_vertex_handle)
      {
...
#pragma omp parallel for schedule(dynamic)
                 ...
                 TargetVertexContainerType> &candidate_set)
    {
      ...
      {
        assert(candidate_set.find(vertex_handle) != candidate_set.end());
      ...
      if constexpr (match_semantics == MatchSemantics::kIsomorphism)
      {
             ...
             map_it++)
        {
          auto [target_matched_it,
                target_matched_ret] = target_matched.emplace(map_it->dst_handle());
        ...
        double query_limit_time = -1.0)
    {
      ...
      std::function<bool(const Match<QueryGraph, TargetGraph> &)>
          par_user_callback = [&result_count,
                               &user_callback,
                               &user_callback_lock,
      ...
      std::function<bool(const Match<QueryGraph, TargetGraph> &)>
          par_prune_callback = [&prune_callback_lock,
                                &prune_callback,
                                //  &user_callback_lock,
                                &user_callback_has_return_false](const MatchType &match_state) -> bool
      {
      ...
      std::map<typename VertexHandle<QueryGraph>::type,
               std::vector<typename VertexHandle<QueryGraph>::type>>
          parent;
      ...
      if constexpr (use_dag_order)
      {
        ...
        if (!partial_match.empty())
        {
               ...
               map_it++)
          {
            ...
            auto [src_vertex_handle_it,
                  src_vertex_handle_ret] = src_vertex_handle_set.emplace(map_it->src_handle());
        ...
        if (src_vertex_handle_set.size() < query_graph.CountVertex())
        {
          ...
          std::set<typename VertexHandle<QueryGraph>::type>
              new_src_vertex_handle_set = src_vertex_handle_set;
               ...
               vertex_it++)
          {
            auto [vertexes_to_add_it,
                  vertexes_to_add_ret] = vertexes_to_add.emplace(vertex_it);
          ...
          while (true)
          {
            ...
            auto remove_vertexes_to_add_callback = [&vertexes_to_add,
#ifndef NDEBUG
                                                    &query_graph,
#endif // NDEBUG
                                                    &vertex_considered](const QueryVertexHandle &query_vertex_handle)
            {
              assert(query_graph.CountVertex() == vertexes_to_add.size() + vertex_considered.size());

              // if (vertexes_to_add.find(query_vertex_handle)
              //  == vertexes_to_add.end()) {
              //   std::cout << "! vertexes_to_add cannot find query_vertex_handle!!"
              //             << std::endl;
              // }
            ...
            // std::cout << "  new_src_vertex_handle_set.size(): "
            ...
            assert(vertexes_to_add.find(next_query_handle) != vertexes_to_add.end());
        ...
        // partial match
        assert(!partial_match.empty() || (src_vertex_handle_set.find(next_query_handle) != src_vertex_handle_set.end()));
        ...
        assert(Bfs<true>(query_graph, src_vertex_handle_set,
                         [](const QueryVertexHandle &query_vertex_handle)
                         { return true; }) == query_graph.CountVertex());
#endif // NDEBUG
      ...
      assert(candidate_set.find(next_query_handle) != candidate_set.end());
      const auto &match_handle_candidate = candidate_set.find(next_query_handle)->second;
      ...
      for (int i = 0; i < match_handle_candidate.size(); i++)
      {
        ...
        if (!kUseFailSet)
        {
          ...
          user_callback_has_return_false = !_dp_iso_using_match::_DPISOUsingMatch<match_semantics, false, use_dag_order>(
              temp_candidate_set,
              temp_match_state,
              temp_target_matched,
              temp_parent,
        ...
        user_callback_has_return_false = !_dp_iso_using_match::_DPISOUsingMatch<match_semantics, true, use_dag_order>(
            temp_candidate_set,
            temp_match_state,
            temp_target_matched,
            temp_parent,
