...
namespace gor
{
                  ...
                  double time_limit = -1.0)
  {
    ...
    assert(candidate_set_for_pivot_vertex.find(pivot) != candidate_set_for_pivot_vertex.end());
    ...
    for (const auto &pivot_candidate_handle : pivot_candidate)
    {

      if (time_limit > 0 && time_limit < (std::time(NULL) - begin_time))
      {
      ...
      assert(pivoted_candidate_set.find(pivot) != pivoted_candidate_set.end());
      ...
      if (!GUNDAM::_dp_iso_using_match ::RefineCandidateSet(gor.pattern(),
                                                            data_graph,
  ...
  size_t IncrementalGORMatch(GraphOracleRule<GraphPatternType,
                             ...
                             const std::vector<typename GUNDAM::VertexHandle<DataGraphType>::type> &delta_target_graph,
                             std::vector<
                                 std::map<typename GUNDAM::VertexHandle<GraphPatternType>::type,
                                          std::vector<typename GUNDAM::VertexHandle<DataGraphType>::type>>> &maximal_match_set)
  {
    ...
    // util::Debug("## here in GORMatch!##");

    assert(candidate_set.find(pivot) != candidate_set.end());
    ...
    for (auto &[query_handle, target_list] : candidate_set)
    {
      ...
      assert(target_list_in_original_graph.size() + target_list_in_delta_graph.size() == target_list.size());
    ...
    for (int mask = 1; mask < kTotalMask; mask++)
    {
           ...
           bit_pos--)
      {
        ...
        assert(pivoted_candidate_set.find(query_vertex_handle) == pivoted_candidate_set.end());
        assert(!target_list_in_delta_graph.empty());
        if (mask & (1 << bit_pos))
        {
          // select target_list_in_delta_graph for this vertex
          auto [pivoted_candidate_set_it,
                pivoted_candidate_set_ret] = pivoted_candidate_set.emplace(query_vertex_handle,
                        ...
                        target_set] : candidate_set)
      {
        ...
        auto pivoted_candidate_set_it = pivoted_candidate_set.find(query_vertex);
        if (pivoted_candidate_set_it != pivoted_candidate_set.end())
        {
      ...
      assert(pivoted_candidate_set.size() == gor.pattern().CountVertex() - 1);
    ...
    for (const auto &pivot_candidate_handle : pivot_candidate)
    {
      ...
      for (const auto &masked_pivoted_candidate_set : total_masked_pivoted_candidate_set)
      {
        CandidateSet pivoted_candidate_set(masked_pivoted_candidate_set);
        assert(pivoted_candidate_set.find(pivot) == pivoted_candidate_set.end());
        auto [pivoted_candidate_set_it,
              pivoted_candidate_set_ret] = pivoted_candidate_set.emplace(pivot,
             ...
             rhs_vertex_idx++)
        {
          ...
          assert(pivoted_candidate_set.find(rhs_vertex_handle) != pivoted_candidate_set.end());
           ...
           rhs_vertex_idx++)
      {
        auto [maximal_match_set_back_it,
              maximal_match_set_back_ret] = maximal_match_set.back().emplace(std::move(rhs_vertex_handle_set[rhs_vertex_idx]),
