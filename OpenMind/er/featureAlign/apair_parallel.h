...
namespace her
{
  ...
  public:
    ...
    std::vector<VertexPair<vertex_t>> Query()
    {
...

      {
        ...
        auto local_vertices_bound = ToChunks(vertices, 1)[world.rank()];
        std::vector<vertex_t> local_vertices(local_vertices_bound.first,
                                             local_vertices_bound.second);
        std::vector<std::thread> threads;
        std::mutex mutex;

        for (auto thread_work_bound : ToChunks(local_vertices, 1))
        {
      ...
      size_t seen_n_points = 0;
      {
        ...
        for (auto &pair : C)
        {
          ...
          raw_result.push_back(VertexPair<vertex_t>(u, v));
          {
            // if (k++ == 4){
            //   break;
            // }
            ...
            if (match)
            {
              result.push_back(VertexPair<vertex_t>(u, v));
            }
    ...
    std::unordered_map<vertex_t, descendants_t> getDesG()
    {
      return s_pair_.getCacheG();
    }

    std::unordered_map<vertex_t, descendants_t> getDesGd()
    {
      return s_pair_.getCacheGd();
    }

    std::vector<VertexPair<vertex_t>> getRawRes()
    {
      return raw_result;
    }
  ...
  private:
    ...
    std::vector<VertexPair<vertex_t>> raw_result;
  };
} // namespace her
