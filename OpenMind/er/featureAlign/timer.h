...
static void timer_next(const std::string &name, double abs_time)
{
  if (timer_enabled)
  {
    timers.emplace_back(std::make_pair(name, -abs_time));
  }
}
  ...
  if (timer_enabled)
  {
    ...
    for (size_t i = 0; i < timers.size() - 1; i++)
    {
      ...
      else
      {
        ...
        for (; j < timers.size(); j++)
        {
          if (timers[j].second > 0)
          {
            break;
          }
        }
        time = timers[j].second - time;
      }
